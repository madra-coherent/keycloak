...
public class RealmCacheSession implements CacheRealmProvider {
    ...
    private Optional<Set<String>> getCachedRoleIdsForClient(ClientModel client) {
        Optional<Set<String>> result = Optional.of(getRolesCacheKey(client.getId()))
            .filter(cacheKey -> !invalidations.contains(cacheKey)
                    && !listInvalidations.contains(client.getId())
                    && !listInvalidations.contains(client.getRealm().getId()))
            .map(cacheKey -> cache.get(cacheKey, RoleListQuery.class))
            .map(query -> query.getRoles());

        if (result.isPresent()) {
            logger.tracev("getClientsRoles cache hit: {0}", client.getClientId());
        }
        return result;
    }

    private void putCachedClientRoleIds(ClientModel client, Set<String> roleIds) {
        String cacheKey = getRolesCacheKey(client.getId());
        Long loaded = cache.getCurrentRevision(cacheKey);
        RoleListQuery query = new RoleListQuery(loaded, cacheKey, client.getRealm(), roleIds, client.getClientId());
        logger.tracev("adding client roles: client {0} key {1}", client.getClientId(), cacheKey);
        cache.addRevisioned(query, startupRevision);
    }
    ...
    public Stream<RoleModel> getRolesByIds(Set<RealmModel> realms, Stream<String> ids) {
        ...
        // Cache lookup
        Map<String, Optional<RoleModel>> rolesCacheLookup = ids.collect(Collectors.toMap(Function.identity(), cacheOp::get));

        // Cache hit
        Stream<RoleModel> cacheHitRoles = rolesCacheLookup.values().stream()
                .filter(Optional::isPresent)
                .map(Optional::get); 

        // Cache miss
        Set<String> missingRolesIds = rolesCacheLookup.entrySet().stream()
                .filter(entry -> !entry.getValue().isPresent())
                .map(Map.Entry::getKey)
                .collect(Collectors.toSet());

        Stream<RoleModel> cacheMissRoles = Stream.empty();
        if (!missingRolesIds.isEmpty()) {
            cacheMissRoles = getRoleDelegate().getRolesByIds(realms, missingRolesIds.stream())
                    .map(ReleaseResource.after(cacheOp::put))
                    .filter(Optional::isPresent)
                    .map(Optional::get);
        }
        ...
        return Stream.concat(cacheHitRoles, cacheMissRoles);
    }
    ...
    public Stream<RoleModel> getRolesByCompositions(Set<RealmModel> realms, Stream<RoleCompositionModel> roleCompositions) {
        ...
        // Cache lookup
        Map<RoleCompositionModel, Optional<RoleModel>> rolesCacheLookup = roleCompositionsById.values().stream()
        ...
        // Cache miss
        Set<RoleCompositionModel> missingRoleCompositions = rolesCacheLookup.entrySet().stream()
                .filter(entry -> !entry.getValue().isPresent())
                .map(Map.Entry::getKey)
                .collect(Collectors.toSet());

        Stream<RoleModel> cacheMissRoles = Stream.empty();
        if (!missingRoleCompositions.isEmpty()) {
            cacheMissRoles = getRoleDelegate().getRolesByCompositions(realms, missingRoleCompositions.stream())
                    .map(r -> new ComposedRoleModel(r, roleCompositionsById.get(r.getId())))
                    .map(ReleaseResource.after(cacheOp::put))
                    .filter(Optional::isPresent)
                    .map(Optional::get);
        }
    ...
    public Stream<String> getClientScopeIdsStream(RealmModel realm) {
        ...
        boolean queryDB = invalidations.contains(cacheKey) || listInvalidations.contains(realm.getId());
        if (queryDB) {
            return getClientScopeDelegate().getClientScopeIdsStream(realm);
        }

        ClientScopeListQuery query = cache.get(cacheKey, ClientScopeListQuery.class);
        return query != null ? query.getClientScopes().stream() : getClientScopeDelegate().getClientScopeIdsStream(realm);
    }
